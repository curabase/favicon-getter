"""Tests for Favicon.find_in_html() method."""
import unittest
...
FALLBACK_URL = 'https://example.com/fallback.png'
...
class TestFindInHtml(unittest.TestCase):
    ...
    def test_data_uri_in_href(self):
        url = self.favicon.find_in_html(FAVICON_AS_DATA_URI, FALLBACK_URL)
        self.assertTrue(url.startswith('data:image'))

    def test_no_favicon_defined(self):
        url = self.favicon.find_in_html(NO_FAVICON_DEFINED, FALLBACK_URL)
        self.assertEqual(url, FALLBACK_URL)

    def test_no_fallback_url_defined(self):
        with self.assertRaises(FavIconException):
            self.favicon.find_in_html('http://example.com/favicon.ico')

    def test_find_in_html_slash_favicon(self):
        html = '<link rel="shortcut icon" href="/favicon.ico">'
        url = self.favicon.find_in_html(html, FALLBACK_URL)
